...
# 初始化 CNN
cnn_model = CNN(input_size=X_train.shape[1], hidden_size=64)
...
results = pd.DataFrame({
    ...
    "RF": [rf_acc, rf_precision, rf_recall, rf_f1, rf_auc, rf_kappa]
})



# 保存 RF 结果到 txt 文件
rf_results_path = "rf_results.txt"
with open(rf_results_path, "w") as f:
    f.write("Random Forest (RF) Model Performance Metrics\n")
    f.write("="*40 + "\n")
    f.write(f"Accuracy: {rf_acc:.4f}\n")
    f.write(f"Precision: {rf_precision:.4f}\n")
    f.write(f"Recall: {rf_recall:.4f}\n")
    f.write(f"F1 Score: {rf_f1:.4f}\n")
    f.write(f"ROC AUC: {rf_auc:.4f}\n")
    f.write(f"Kappa: {rf_kappa:.4f}\n")

print(f"RF results saved to {rf_results_path}")
