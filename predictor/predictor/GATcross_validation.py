...
# 新的GAT细胞特征文件路径
GAT_CELL_FEAT_FILE = "/public/home/Yule1/Desktop/DeepLearning/PPI/cell/data/GATcell_feat.npy"
# 输出转换后的cell特征文件路径，替换原CELL_FEAT_FILE
CELL_FEAT_FILE = "/public/home/Yule1/Desktop/DeepLearning/PPI/cell/data/cell_feat.npy"

def process_gat_features(gat_feat_path, output_path, target_dim=768):
    print(f"正在加载 {gat_feat_path} 并进行线性投影到 {target_dim} 维...")
    # 加载 GATcell_feat.npy，预期形状为 (2, 39, 256)
    gat_feat = np.load(gat_feat_path)
    if gat_feat.ndim != 3 or gat_feat.shape[0] != 2:
        raise ValueError("GATcell_feat.npy 的形状不符合预期，应为 (2, 39, 256)")
    # 合并两个通道，得到形状 (39, 512)
    combined_feat = np.concatenate((gat_feat[0], gat_feat[1]), axis=1)
    # 使用线性投影将 512 维映射到 target_dim（768）维
    # 这里采用固定随机权重以保证可复现
    np.random.seed(0)
    W = np.random.randn(combined_feat.shape[1], target_dim)  # shape: (512, 768)
    projected_feat = np.dot(combined_feat, W)  # shape: (39, 768)
    # 标准化
    scaler = StandardScaler()
    projected_feat = scaler.fit_transform(projected_feat)
    # 保存转换后的特征
    np.save(output_path, projected_feat)
    print(f"转换完成，已保存到 {output_path}")

# 执行转换，覆盖原有cell_feat.npy
process_gat_features(GAT_CELL_FEAT_FILE, CELL_FEAT_FILE, target_dim=768)
        ...
        for hs in args.hidden:
            for lr in args.lr:
                ...
                for valid_fold in outer_trn_folds:
                    ...
                    logging.info("Start inner loop: train folds {}, valid folds {}".format(inner_trn_folds, valid_folds))
