...
class NOFastSynergyDataset(Dataset):

    def __init__(self, drug2id_file, cell2id_file, drug_feat_file, cell_feat_file, synergy_score_file, use_folds, train=True):
        # 读取映射文件和特征文件
        self.drug2id = read_map(drug2id_file)
        self.cell2id = read_map(cell2id_file)
        self.drug_feat = np.load(drug_feat_file)
        self.cell_feat = np.load(cell_feat_file)
        self.samples = []  # 存储处理后的样本
        self.raw_samples = []  # 存储原始样本
        self.train = train  # 是否为训练模式

        valid_drugs = set(self.drug2id.keys())  # 有效的药物ID
        valid_cells = set(self.cell2id.keys())  # 有效的细胞ID
        
        # 读取已有标签的组合，记录哪些药物组合和细胞已经有标签
        labeled_combinations = set()
        with open(synergy_score_file, 'r') as f:
            f.readline()  # 跳过标题行
            for line in f:
                drug1, drug2, cellname, score, fold = line.rstrip().split('\t')
                if int(fold) in use_folds:  # 只处理指定折叠的数据
                    if drug1 in valid_drugs and drug2 in valid_drugs and cellname in valid_cells:
                        labeled_combinations.add((self.drug2id[drug1], self.drug2id[drug2], self.cell2id[cellname]))

        # 获取所有药物对的组合（正反两个方向）
        drug_combinations = list(itertools.combinations(valid_drugs, 2))
        drug_pairs = set()
        for drug1, drug2 in drug_combinations:
            drug_pairs.add((self.drug2id[drug1], self.drug2id[drug2]))  # 正向组合
            drug_pairs.add((self.drug2id[drug2], self.drug2id[drug1]))  # 反向组合

        # 遍历药物对和细胞，生成所有可能的组合
        for drug1_id, drug2_id in drug_pairs:
            for cellname, cell_id in self.cell2id.items():
                # 如果该药物组合-细胞的组合已经有标签，则跳过
                if (drug1_id, drug2_id, cell_id) not in labeled_combinations:
                    # 如果没有标签，则创建样本
                    sample = [
                        torch.from_numpy(self.drug_feat[drug1_id]).float(),
                        torch.from_numpy(self.drug_feat[drug2_id]).float(),
                        torch.from_numpy(self.cell_feat[cell_id]).float(),
                        torch.FloatTensor([0.0]),  # 协同作用分数（未标签化，设为0.0）
                    ]
                    self.samples.append(sample)
                    raw_sample = [drug1_id, drug2_id, cell_id, '0.0']  # 记录原始样本
                    ...
                    if train:
                        ...
                        self.samples.append(sample)
                        raw_sample = [drug2_id, drug1_id, cell_id, '0.0']
                        self.raw_samples.append(raw_sample)
...
class FastSynergyDataset(Dataset):
    def __init__(self, drug2id_file, cell2id_file, drug_feat_file, cell_feat_file, synergy_score_file, use_folds, train=True):
        self.drug_feats = np.load(drug_feat_file)
        self.cell_feats = np.load(cell_feat_file)
        
        # 打印数据形状以检查维度是否正确
        print(f"Drug feature shape: {self.drug_feats.shape}")
        print(f"Cell feature shape: {self.cell_feats.shape}")

        # 允许更少的样本通过
        if self.drug_feats.shape[0] < 2 or self.cell_feats.shape[0] < 2:
            print("Warning: Data array has insufficient elements, but proceeding anyway.")
        ...
        # 使用 synergy_score_file 路径读取协同作用数据
        with open(synergy_score_file, 'r') as f:
            f.readline()  # 跳过标题行
            for line in f:
                drug1, drug2, cellname, score, fold = line.rstrip().split('\t')
                if drug1 in valid_drugs and drug2 in valid_drugs and cellname in valid_cells:
                    if int(fold) in use_folds:
                        # 检查 cell_id 是否超出范围
                        if self.cell2id[cellname] < len(self.cell_feat):  # 确保细胞索引有效
                            sample = [
