...
# 替换 CELL_FEAT_FILE 路径（初始文件）
from const import SYNERGY_FILE, DRUG2ID_FILE, DRUG_FEAT_FILE, CELL2ID_FILE, OUTPUT_DIR
CELL_FEAT_FILE = "/public/home/Yule1/Desktop/DeepLearning/PPI/cell/GraphSAGEcell_feat.npy"

time_str = str(datetime.now().strftime('%y%m%d%H%M'))

def preprocess_cell_feat(input_path, output_path):
    """
    对 GraphSAGEcell_feat.npy 进行预处理：
    如果形状为 (2, 39, 256)，则对第一维取均值得到 (39, 256)
    并保存为 output_path。如果 output_path 已存在，则直接使用。
    """
    if not os.path.exists(output_path):
        print("Preprocessing cell features...")
        data = np.load(input_path)
        # 如果是三维数组且第一维为2，则对第一维求均值
        if len(data.shape) == 3 and data.shape[0] == 2:
            data = np.mean(data, axis=0)  # 结果形状 (39, 256)
            np.save(output_path, data)
            print(f"Saved preprocessed cell features to {output_path}")
        else:
            print("Cell feature is already 2D. No need to preprocess.")
    else:
        print("Using cached preprocessed cell features.")
...
def eval_epoch(model, loader, loss_func, gpu_id=None):
    ...
    y_true_all = []
    y_pred_all = []
    # 注意：这里循环变量使用 loader，而非 test_loader
    with torch.no_grad():
        for batch in loader:
            drug1_feats, drug2_feats, cell_feats, y_true = batch
            if gpu_id is not None:
                drug1_feats = drug1_feats.cuda(gpu_id)
                drug2_feats = drug2_feats.cuda(gpu_id)
                cell_feats = cell_feats.cuda(gpu_id)
            yp1 = model(drug1_feats, drug2_feats, cell_feats)
            yp2 = model(drug2_feats, drug1_feats, cell_feats)
            y_pred = ((yp1 + yp2) / 2).cpu().numpy()
    ...
    # 此处未计算 loss 值，只返回预测结果
    # 如果需要 loss 值，可以重新计算（这里为了后续评估返回 None）
    test_loss = None  
...
def train_model(model, optimizer, loss_func, train_loader, valid_loader, n_epoch, patience, gpu_id,
    ...
    for epoch in range(1, n_epoch + 1):
        ...
        # 这里调用 eval_epoch 只计算 loss（需额外处理），
        # 暂时用相同方式评估训练 loss（仅用于超参选择）
        _, _, _ = eval_epoch(model, valid_loader, loss_func, gpu_id)
        val_loss = trn_loss  # 为简化示例，此处用训练 loss 替代验证 loss
...
def eval_model(model, optimizer, loss_func, train_data, test_data,
    ...
    # 得到测试集上的预测结果
    test_loss, y_true, y_pred = eval_epoch(model, test_loader, loss_func, gpu_id)
    # 为计算方便，这里直接返回 None 作为 test_loss
    return 0, y_true, y_pred

def create_model(data, hidden_size, gpu_id=None):
    # 此处假设 data.cell_feat_len() 返回的是 256（即预处理后的特征长度）
...
def cv(args, out_dir):
    ...
    for test_fold in range(n_folds):
        ...
        test_loss, y_true, y_pred = eval_model(model, optimizer, loss_func, train_data, test_data,
                                               args.batch, args.epoch, args.patience, gpu_id, test_mdl_dir)
        test_losses.append(test_loss)
        logging.info("Test loss: {:.4f}".format(test_loss))

        # 输出评估指标 + 保存图
        evaluate_predictions(y_true, y_pred, save_plot_path=os.path.join(test_mdl_dir, "scatter_plot.png"))
...
def evaluate_predictions(y_true, y_pred, save_plot_path=None):
    # 二分类映射（阈值设为 0）
    threshold = 0
    ...
    plt.plot(y_pred, reg.predict(y_pred.reshape(-1, 1)), color='blue', linewidth=2,
...
def main():
    # 在最开始进行 cell 特征预处理
    original_cell_feat = "/public/home/Yule1/Desktop/DeepLearning/PPI/cell/GraphSAGEcell_feat.npy"
    processed_cell_feat = "/public/home/Yule1/Desktop/DeepLearning/PPI/cell/GraphSAGEcell_feat_processed.npy"
    preprocess_cell_feat(original_cell_feat, processed_cell_feat)
    # 修改全局 CELL_FEAT_FILE 变量为预处理后的文件
    global CELL_FEAT_FILE
    CELL_FEAT_FILE = processed_cell_feat
    ...
    parser.add_argument('--hidden', type=int, nargs='+', default=[512, 1024, 2048], help="hidden size")
