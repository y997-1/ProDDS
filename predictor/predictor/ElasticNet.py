...
# 训练 Elastic Net（弹性网络）
elastic_net_model = SGDClassifier(loss="log_loss", penalty="elasticnet", l1_ratio=0.5, max_iter=1000, random_state=42)
elastic_net_model.fit(X_train, y_train)

# Elastic Net 预测
y_pred_en = elastic_net_model.predict(X_test)
y_pred_en_prob = elastic_net_model.predict_proba(X_test)[:, 1]
...
en_auc = roc_auc_score(y_test, y_pred_en_prob)
en_kappa = cohen_kappa_score(y_test, y_pred_en)

# 显示结果
results = pd.DataFrame({
    "Metric": ["Accuracy", "Precision", "Recall", "F1 Score", "ROC AUC", "Kappa"],
    "CNN": [cnn_acc, cnn_precision, cnn_recall, cnn_f1, cnn_auc, cnn_kappa],
    "Elastic Net": [en_acc, en_precision, en_recall, en_f1, en_auc, en_kappa]
})

# 保存 Elastic Net 结果到 txt 文件
en_results_path = "elastic_net_results.txt"
with open(en_results_path, "w") as f:
    f.write("Elastic Net Model Performance Metrics\n")
    f.write("="*40 + "\n")
    f.write(f"Accuracy: {en_acc:.4f}\n")
    f.write(f"Precision: {en_precision:.4f}\n")
    f.write(f"Recall: {en_recall:.4f}\n")
    f.write(f"F1 Score: {en_f1:.4f}\n")
    f.write(f"ROC AUC: {en_auc:.4f}\n")
    f.write(f"Kappa: {en_kappa:.4f}\n")

print(f"Elastic Net results saved to {en_results_path}")
