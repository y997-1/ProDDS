...
class GraphSAGEEncoder(nn.Module):
    def __init__(self, in_features: int, out_features: int, activation=F.relu, k: int = 2, aggregator_type='mean'):
        super(GraphSAGEEncoder, self).__init__()
        assert k >= 2, "GraphSAGE must have at least 2 layers"
        ...
        # 第一层 GraphSAGE
        self.conv.append(SAGEConv(in_features, 2 * out_features, aggregator_type=aggregator_type))

        # 中间层 GraphSAGE
        for _ in range(1, k - 1):
            self.conv.append(SAGEConv(2 * out_features, 2 * out_features, aggregator_type=aggregator_type))

        # 最后一层 GraphSAGE
        self.conv.append(SAGEConv(2 * out_features, out_features, aggregator_type=aggregator_type))
...
class Cell2Vec(nn.Module):
    ...
    def __init__(self, encoder: GraphSAGEEncoder, n_cell, n_dim):
